 ...
 * @author azizk
 */
public class StatConseilController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
     ...
     public void calcul() {
        ConseilService ser= new ConseilService();
        int np = 0 , nv=0 ,k=0; 
        List<Conseil> li =ser.ListClasse();
        int i = 0; 
        
        for ( i=0 ; i<li.size();i++){
        if (li.get(i).getTypecon().equals("Plastique"))
        
        nv=nv+1;  
       else if (li.get(i).getTypecon().equals("Verre"))
            np=np+1 ;
    ...
    private void back(ActionEvent event) throws IOException {
        ...
        loader.setLocation(getClass().getResource("/gui/AfficherConseil.fxml"));
